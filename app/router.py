...
# The workbench owns these addresses: every router is mounted here, always, whether the category
# that shows it is switched on or not. Which category is on lives in `config/settings.json`, and
# `/plugins.js` only decides what the sidebar lists. Mounting from the plugin registry instead —
# what phase 2 did — made one category depend on another: five views outside LLM Pool read
# `/api/models`, and they lost it when LLM Pool was off.
