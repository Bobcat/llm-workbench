# Reviewprompt PR #20 — de vier open gaten uit sectie 4

Review PR #20, branch `feature/open-gaps-cleanup` tegen `main`, in
`/home/gunnar/projects/llm-workbench`.

Schrijf je bevindingen naar:

`docs/reviews/pr-20-open-gaps-cleanup-review-findings-1.md`
...
1. **Punt b — helpt het paneel nu echt?** Het paneel verschijnt in twee gevallen: de lijst komt niet
...
2. **Punt c — dekt de terugval de gevallen, en breekt hij niets?** Wat gebeurt er bij een deep link,
...
3. **Punt a — is het gedrag van de zes dienstmodules echt onveranderd?** Vergelijk de nieuwe
...
4. **Punt d — klopt de regel voor beide wortels?** Een pad-icoon is nu `plugin-static/<id>/…` voor een
...
5. **Wat is er niet af?** Noem wat er van deze vier punten blijft liggen. Het splitsen van
...
De pytest-suite geeft 197 passed en 5 failures in `tests/test_replay_api.py`; die vijf falen ook op
...
`docs/reviews/pr-20-open-gaps-cleanup-review-findings-1.md`. Beantwoord expliciet of de vier punten
af zijn, of punt a het gedrag van de diensten niet heeft veranderd, en of punt c niets heeft gebroken.
