...
import { isPluginAssetPath } from './plugin-assets.js';
...
export function iconMarkup(name, className = '') {
  ...
  // A plugin's own icon file: resolved against the document base, like a view module, so it also
  // works under a subpath. Anything else is a sprite symbol, and that allowlist is what keeps this
  // value safe in markup.
  if (isPluginAssetPath(value)) {
