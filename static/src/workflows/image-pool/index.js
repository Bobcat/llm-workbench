...
const IMAGE_POOL_ADDRESS_LABEL = '@127.0.0.1:8030';
...
export function createImagePoolView() {
  ...
  async function refreshModels(options = {}) {
    ...
    try {
      const [modelsPayload, gpuPayload] = await Promise.all([
        api.getImageAdminModels(),
        api.getImageAdminGpuMemory().catch(() => null),
      ]);
  ...
  async function runAction(modelName, kind) {
    ...
    try {
      if (kind === 'load') {
        await api.loadImageAdminModel(modelName);
      } else if (kind === 'unload') {
        await api.unloadImageAdminModel(modelName);
      }
      lastError = '';
    } catch (err) {
      lastError = formatApiError(err);
...
function buildRowsMarkup(models, expandedModels, activeActionModel, activeActionKind) {
  ...
  return models.map((model, index) => {
    ...
    const inputTypes = formatList(model.capabilities?.input_mime_types);
    const outputTypes = formatList(model.capabilities?.output_mime_types);
    const tasks = formatList(model.capabilities?.tasks);
    ...
    return `
      ...
      >
        <div class="llm-pool-definition-grid">
          ...
          <div><span>Tasks</span><strong>${escapeHtml(tasks)}</strong></div>
          <div><span>Input types</span><strong>${escapeHtml(inputTypes)}</strong></div>
          <div><span>Output types</span><strong>${escapeHtml(outputTypes)}</strong></div>
...
function buildCapabilitiesSummary(capabilities) {
  const parts = [];
  const tasks = Array.isArray(capabilities?.tasks) ? capabilities.tasks.length : 0;
  if (tasks > 0) {
    parts.push(`${tasks} tasks`);
  }
  const inputTypes = Array.isArray(capabilities?.input_mime_types) ? capabilities.input_mime_types.length : 0;
  if (inputTypes > 0) {
    parts.push(`${inputTypes} input types`);
  }
  const outputTypes = Array.isArray(capabilities?.output_mime_types) ? capabilities.output_mime_types.length : 0;
  if (outputTypes > 0) {
    parts.push(`${outputTypes} output types`);
  }
  const description = String(capabilities?.description || '').trim();
  if (description) {
